    ...
    @section('ModificarAlumnosD')
        ...
        <form class="form-group" method="PUT" action="/ModificarAlumno/{{$alumno->Clave_A}}/edit">
            ...
            <div style="position: absolute;top: 62%; left: 24.9%; width: 25%;height:53%;background-color:#aaa">
                <p><input type="text" required readonly pattern="[1-9]{2}[0]{2}[1]{2}[0-9]{4}" name="Clave_A" value="{{$alumno->Clave_A}}" style="font-size:105%; width: 95%"/></p>
                ...
                <p><input type="text" required readonly id="nombremadre" pattern="[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ,.'-]{2,48}" name="Nombre_M" value="{{$alumno->Nombre_M}}"style="font-size:105%; width: 95%;"/></p>

                <p><input type="text" required readonly id="nombrpadre" pattern="[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ,.'-]{2,48}" name="Nombre_P" value="{{$alumno->Nombre_P}}"style="font-size:105%; width: 95%;"/></p>

                <p><input type="text"required readonly pattern="[1-9]{1}[0-9]{9}" id="telefonotutor" name="Telefono_T"  value="{{$alumno->Telefono_T}}" style="font-size:105%; width: 95%;"/></p>

                <p><input type="tel" required readonly id="telefonoalumno" name="Telefono_A"
            ...
            <div style="position: absolute;top: 62%; left: 70%; width:25%;height:53%;background-color:#aaa">
                <p><input type="text" required readonly id="municipio" pattern="[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ,.'-]{2,48}" name="Municipio" value="{{$alumno->Municipio}}" style="font-size:105%; width:97%"/></p>
                <p><input type="text" required readonly id="poblacion" pattern="[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ,.'-]{2,48}" name="Poblacion" value="{{$alumno->Poblacion}}" style="font-size:105%; width:97%" /></p>
                ...
                <p><input type="text" required readonly id="curp" pattern="^([A-Z][AEIOUX][A-Z]{2}\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])[HM](?:AS|B[CS]|C[CLMSH]|D[FG]|G[TR]|HG|JC|M[CNS]|N[ETL]|OC|PL|Q[TR]|S[PLR]|T[CSL]|VZ|YN|ZS)[B-DF-HJ-NP-TV-Z]{3}[A-Z\d])(\d)$" name="Curp" value="{{$alumno->Curp}}"  value="{{$alumno->Curp}}" style="font-size:105%; width:97%;"/></p>
                <p><input type="text" id="nss"  value="{{$alumno->NSS}}" min="1" max="99999999999" name="NSS" style="font-size:105%; width:97%;"/></p>

                <p> <input type="date" name="Fecha_Nac" required readonly value="{{$alumno->Fecha_Nac}}" style="width:90%"> </p>


                <p> <select name="Edad" required disabled style="width: 20%">
            ...
            <div style="font-size:140%;width: 90%; height: 11.5%;position: absolute;top:120%;left:5%; background-color:#aaa">
                    <p style="position: absolute;top: 3px;left:170px; width: 900px">{{('Con todo respeto solicito a Usted Sr. Director, me acepte como alumno  de esta Escuela Preparatoria a su cargo para cursar el')}}</p>
                        <select name="Grado" disabled onchange="Grados(this.value); semes(this.options[this.selectedIndex].innerHTML);" required="true" style="font-size:80%;width: 17%;/*posicion->*/position: absolute;top:54%; left:41%;">
